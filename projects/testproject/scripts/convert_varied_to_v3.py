...
def add_episode(
    ...
    for frame_index in tqdm(range(5, num_frames), desc="Processing each frame"):
        ...
        # LOCAL PATCH (2026-08-07): state-machine recordings store 8-dim
        # EE-POSE actions, but a policy that must drive the real arm needs
        # 6-dim JOINT actions. The recording already contains the right
        # signal: obs/joint_pos_target is the controller's commanded joint
        # position each step - identical semantics to a leader-teleop
        # dataset's action. Override the frame's action with it.
        tgt = all_data["obs"]["joint_pos_target"][frame_index]
        frame["action"] = tgt.cpu().numpy() if hasattr(tgt, "cpu") else tgt
        if task is not None:
            frame["task"] = task
