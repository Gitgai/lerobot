...
torch.manual_seed(0)
ref_m = torch.nn.Sequential(torch.nn.Linear(256, 256), torch.nn.GELU(),
...
for i in range(10):
    ...
    for m_, o_ in ((ref_m, ref_o), (off_m, off_o)):
        o_.zero_grad(); m_(x).square().mean().backward(); o_.step()

md = max((a - b).abs().max().item()
         for a, b in zip(ref_m.parameters(), off_m.parameters()))
