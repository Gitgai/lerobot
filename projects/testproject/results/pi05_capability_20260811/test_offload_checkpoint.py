...
def describe(tag, o):
    ...
    ea = st["exp_avg"]
    print(
        f"  {tag:9} dtype={str(ea.dtype):16} device={str(ea.device):6} "
...
print("=" * 70)
print("CPUOffloadAdamW  ->  lerobot checkpoint round-trip")
print("=" * 70)
...
if after.dtype != torch.float32:
    print(f"  ⛔ DTYPE LOST: fp32 -> {after.dtype}. The fp32-state experiment is void.")
    ok = False
if after.device.type != "cpu":
    print(f"  ⛔ STATE MOVED TO {str(after.device).upper()}: offload defeated on resume.")
    ok = False
    print(
        f"     at pi05 scale that is 4.14B x 2 x {after.element_size()}B = "
        f"{4.14e9 * 2 * after.element_size() / 1e9:.1f} GB landing on a 32 GB card."
    )
elif not after.is_pinned():
    print("  ⚠ UNPINNED on resume: correct, but transfers fall back to a staged")
    ok = False
